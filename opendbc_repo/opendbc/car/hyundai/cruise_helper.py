...
def enable_radar_tracks(CP, can_recv, can_send):
  ...
  if CP.openpilotLongitudinalControl and CP.carFingerprint in [CAR.HYUNDAI_NEXO]:
    rdr_fw = None
    ...
    if True:
      ...
      try:
        for i in range(40):
          try:
            query = IsoTpParallelQuery(can_send, can_recv, CP.sccBus, [rdr_fw_address], [b'\x10\x07'], [b'\x50\x07'], debug=True)
            for addr, dat in query.get_data(0.1).items(): # pylint: disable=unused-variable
              ...
              query = IsoTpParallelQuery(can_send, can_recv, CP.sccBus, [rdr_fw_address], [WRITE_DAT_REQUEST+dataId+new_config], [WRITE_DAT_RESPONSE], debug=True)
              query.get_data(0)
              print(f"Try {i+1}")
              break
            break
